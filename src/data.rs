...
use std::io::{Write, Result, Error, copy};
use ron;
use url;
use reqwest;
...
fn make_html(texts: &HashMap<String, DlItems>) -> String {
    ...
    for (year, item) in texts {
        ...
        for (state, url) in &item.formal_prefs {
            listy.push_str(&format!("<li><a href=\"{}\">Formal Preferences for {}</a></li>\n", url, state));
        }
        content.push_str(&template_list.replace("LIST_ITEMS", &listy).replace("YEAR", &year));
    }
...
pub fn examine_html(filey: &Path){
    ...
    output.write(make_html(&sacred_texts).as_bytes()).expect("Error writing file");
}
    ...
    for (_, item) in sacred_texts {
        ...
        for link in all_urls {
            ...
            if !dlto.is_file() {
                ...
                let response = reqwest::blocking::get(&link).expect(&format!("Error downloading {:#?}", &aspath.file_name().unwrap())).bytes().unwrap();
