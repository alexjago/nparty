...
use std::path;
use std::str::FromStr;
use zip::{read::ZipFile, result::ZipResult, ZipArchive, ZipWriter};
use SeekFrom::Start;
...
/// Totally not a [ZipFile].
///
/// We want [open_csvz] to avoid having to just read the entire thing into memory.
/// The issue is that [ZipFile]s have a lifetime reference to their parent [ZipArchive]s
/// Solution: a self-referential struct of both, and then `impl Read` to delegate to the [ZipFile]...
/// Many thanks to ark#7419 and also to kpreid#9810, Stargateur#1337 and shepmaster#6139 from the Rust Discord
#[self_referencing]
pub struct TotallyNotAZipFile<R: Read + Seek + Send + 'static> {
...
impl<R: Read + Seek + Send + 'static> TotallyNotAZipFile<R> {
    /// Construct the struct.
    /// (can't call it "new" since the name collides with a fn generated by ouroboros)
    /// TODO have this be intelligent rather than hardcoding entry zero
    pub fn construct(reader: R) -> ZipResult<Self> {
        TotallyNotAZipFileTryBuilder {
            archive: ZipArchive::new(reader)?,
            file_builder: |archive| archive.by_index(0),
        }
        .try_build()
    }
}

impl<R: Read + Seek + Send> Read for TotallyNotAZipFile<R> {
...
/// If the zip archive contains more than one file, the zeroth will be returned.
/// Performance note: has to unzip and return the entire file.
pub fn open_csvz<T: 'static + Read + Seek + Send>(mut infile: T) -> Result<Box<dyn Read>> {
    if is_zip(&mut infile)? {
        Ok(Box::new(TotallyNotAZipFile::construct(infile)?))
    } else {
        Ok(Box::new(infile))
    }
}
