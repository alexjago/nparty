...
fn main() -> anyhow::Result<()> {
    ...
    match m.command {
        ...
        Data(sm) => match sm {
            Download {DL_FOLDER} => data::download(&DL_FOLDER),
            Examine {FILE} => FILE.filter(|x| x.exists()).map_or_else(
                data::examine_txt,
...
fn run(args: CliRun) -> anyhow::Result<()> {
...
fn do_upgrade_prefs(args: CliUpgradePrefs) -> anyhow::Result<()> {
...
fn do_configure(args: CliConfigure) -> anyhow::Result<()> {
    // requireds
    let candspath = args.candidates;
