...
const PREFS_FIELD_NAMES : [&str; 6] = ["State", "Division", "Vote Collection Point Name", "Vote Collection Point ID", "Batch No", "Paper No"];
...
pub fn booth_npps(parties: &Parties, state: &StateAb, formal_prefs_path: &Path, polling_places_path: &Path, npp_booths_path: &Path){ // cut down for WIP-ing
    ...
    let mut pp_rdr = csv::ReaderBuilder::new().flexible(true).has_headers(false).from_path(polling_places_path).unwrap();
    ...
    loop {
        if let Some(result) = pp_rdr_iter.next() {
            ...
            let dvb = (record.DivisionNm.to_owned(), record.PollingPlaceNm.to_owned());
    ...
    let mut prefs_rdr = csv::ReaderBuilder::new().flexible(true).escape(Some(b'\\'))
    ...
    //eprintln!("\nATL start: {}\tBTL start: +{}", atl_start, btl_start);
    ...
    for row in prefs_rdr_iter {
        ...
        for i in atl_start..prefs_headers.len() {
            if i >= record.len(){
                seq.push(cands_count);
            } else {
                let pref = match record.get(i).unwrap().parse::<usize>() {
                    Ok(n) => n,
                    Err(_) => cands_count,
                };
                seq.push(pref);
        ...
        // originally this condition was (btl_start > 0) but that breaks if there are no ATLs
        // instead, we bail early if there aren't enough fields in this row to contain any BTLs.
        let mut is_btl = seq.len() >= (atl_start + btl_start);
        if is_btl {
            let mut btl_counts: [usize; 7] = [0; 7]; // use an extra slot, simplify indexing
            for x in seq[btl_start..seq.len()].iter(){
                for j in 1..=6 {
                    if *x == j {
                        btl_counts[j] += 1;
                    }
        ...
        for (p, cns) in groups_which.iter(){
            ...
            for i in cns{
                if seq[i - &1_usize] < curbest {
                    curbest = seq[i - &1_usize];
                }
            }
        ...
        let booth = booth_counts.entry(divbooth).or_insert(vec![0_usize; combinations.len()]);
        booth[pref_idx] += 1;
        ...
        if progress % 100000 == 0 {
            eprintln!("{}\t\tPreferencing progress: {} ballots", term::TTYJUMP, progress);
        }
    }

    // and we are done with the main task!
    eprintln!("{}\t\tPreferencing complete: {} ballots", term::TTYJUMP, progress);
    ...
    let mut wtr = csv::WriterBuilder::new().terminator(csv::Terminator::CRLF).has_headers(false).from_path(npp_booths_path).unwrap();
    ...
    wtr.write_record(npp_header).expect("error writing booths header");

    for (bk, bv) in booth_counts.iter().sorted(){
        let br = booths.get(bk).expect(&format!("It's really weird, but {:#?} isn't in `booths`.", bk));
        ...
        bdeets.push(match br.Latitude {
            Some(x) => x.to_string(),
            None => "".to_string(),
        });
        bdeets.push(match br.Longitude {
            Some(x) => x.to_string(),
            None => "".to_string(),
        });
        let mut total = 0;
